...
if __name__ == '__main__':
    ...
    print('reading file %s' % str(Path(args.save_dir).joinpath('tmp_model_params1', args.model_type, args.pred, args.feature_type, args.adv_dataset, model_setting_str)))
    for shadow_idx in range(5):
        for epoch in range(int(args.num_epochs)):
            adv_federated_model_result_path = Path(args.save_dir).joinpath('tmp_model_params1', args.model_type, args.pred, args.feature_type, args.adv_dataset, model_setting_str, 'fold'+str(int(shadow_idx+1)))
    ...
    for fold_idx in range(1):
        ...
        for epoch in range(int(args.num_epochs)):
            ...
            # Model related
            federated_model_result_path = Path(args.save_dir).joinpath('tmp_model_params1', args.model_type, args.pred, args.feature_type, args.dataset, model_setting_str, 'fold'+str(int(fold_idx+1)))
