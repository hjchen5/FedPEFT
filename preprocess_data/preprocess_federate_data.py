...
def write_data_dict(tmp_dict, sentence_file, speaker_id, label, gender, file_path):
    tmp_dict['sentence_file'], tmp_dict['speaker_id'], tmp_dict['gender'], tmp_dict['file_path'], tmp_dict[
        'label'] = sentence_file, speaker_id, gender, file_path, label
...
def save_data_dict(train_speaker_id_arr, test_speaker_id_arr, sentence_file, speaker_id, label, gender, file_path):
    if speaker_id in test_speaker_id_arr:
        test_dict[sentence_file] = {}
        write_data_dict(test_dict[sentence_file], sentence_file, speaker_id, label, gender, file_path)
    elif speaker_id in train_speaker_id_arr:
        training_dict[sentence_file] = {}
        write_data_dict(training_dict[sentence_file], sentence_file, speaker_id, label, gender, file_path)


def save_data_dict_msp(train_speaker_id_arr, test_speaker_id_arr, sentence_file, speaker_id, label, gender, file_path):
    if speaker_id in test_speaker_id_arr:
        test_dict[sentence_file] = {}
        write_data_dict(test_dict[sentence_file], sentence_file, 'msp_improv_' + speaker_id, label, gender, file_path)
    elif speaker_id in train_speaker_id_arr:
        training_dict[sentence_file] = {}
        write_data_dict(training_dict[sentence_file], sentence_file, 'msp_improv_' + speaker_id, label, gender,
                        file_path)
    ...
    for train_index, test_index in kf.split(speaker_id_arr):
        ...
        if data_set_str == 'msp-improv':
            ...
            for sentence_file in tqdm(sentence_file_list, ncols=100, miniters=100):
                ...
                file_path = Path('/home/chenhj/WorkSpaces_python/peft-ser-main/audio/').joinpath(args.dataset,
                                                                                                 f'{sentence_file}.wav')
                ...
                save_data = np.array(data['data'])[0] if args.feature_type == 'emobase' else np.array(data['data'])[0,
                                                                                             0, :].flatten()
                save_data_dict_msp(train_speaker_id_arr, test_speaker_id_arr, sentence_file, speaker_id, label, gender,
                                   str(file_path))

        elif data_set_str == 'crema-d':

            # speaker id for training and test
            train_speaker_id_arr, test_speaker_id_arr = [tmp_idx for tmp_idx in train_arr], [tmp_idx for tmp_idx in
                                                                                             test_arr]
            print('Train speaker:')
            print(train_speaker_id_arr)
            print('Test speaker:')
            ...
            # data root folder
            demo_df = pd.read_csv(str(Path(args.data_dir).joinpath('processedResults', 'VideoDemographics.csv')),
                                  index_col=0)
            rating_df = pd.read_csv(str(Path(args.data_dir).joinpath('processedResults', 'summaryTable.csv')),
                                    index_col=1)
            sentence_file_list = list(Path(args.data_dir).joinpath('AudioWAV').glob('*.wav'))
            sentence_file_list.sort()

            for sentence_file in tqdm(sentence_file_list, ncols=100, miniters=100):
                if '1076_MTI_SAD_XX.wav' in str(sentence_file): continue
                ...
                file_path = Path('/home/chenhj/WorkSpaces_python/peft-ser-main/audio/').joinpath(args.dataset,
                                                                                                 f'{sentence_file}.wav')
                save_data_dict(train_speaker_id_arr, test_speaker_id_arr, sentence_file, speaker_id, label, gender,
                               str(file_path))

        elif data_set_str == 'iemocap':
            # speaker id for training, validation, and test
            speaker_id_list = ['Ses01F', 'Ses01M', 'Ses02F', 'Ses02M', 'Ses03F', 'Ses03M', 'Ses04F', 'Ses04M', 'Ses05F',
                               'Ses05M']
            train_speaker_id_arr = [speaker_id_list[tmp_idx] for tmp_idx in train_arr]
            test_speaker_id_arr = [speaker_id_list[tmp_idx] for tmp_idx in test_arr]
            print('Train speaker:')
            ...
            for session_id in ['Session1', 'Session2', 'Session3', 'Session4', 'Session5']:
                ground_truth_path_list = list(
                    Path(args.data_dir).joinpath(session_id, 'dialog', 'EmoEvaluation').glob('*.txt'))
                    ...
                    with open(str(ground_truth_path)) as f:
                        ...
                        for line in label_lines:
                            if 'Ses' in line:
                                ...
                                label = line.split('\t')[-2]
                                # chenhj 音频使用peft-ser-main处理后的
                                file_path = Path(args.data_dir).joinpath(
                                    session_id, 'sentences', 'wav', '_'.join(sentence_file.split('_')[:-1]),
                                    f'{sentence_file}.wav'
                                )
                                # file_path = Path('/home/chenhj/WorkSpaces_python/peft-ser-main/audio/').joinpath(
                                #     args.dataset, f'{sentence_file}.wav')
                                data = data_dict[sentence_file]
                                save_data = np.array(data['data'])[0] if args.feature_type == 'emobase' else np.array(
                                    data['data'])[0, 0, :].flatten()

                                # if 'impro' not in line: continue
                                if label == 'ang' or label == 'neu' or label == 'sad' or label == 'hap' or label == 'exc':
                                    if label == 'exc': label = 'hap'
                                    save_data_dict(train_speaker_id_arr, test_speaker_id_arr, sentence_file, speaker_id,
                                                   label, gender, file_path)
